describe("login Automation Tests", () => {
    ...
    it("Test Case 02-03 - Validate the logout", () => {
        "login/logout", () => {
            const login = ["TechGlobal", "Test1234"]
            cy.get("#username, #password").each(($el, index) => {
                cy.wrap($el)
...
it("Test Case 04-05 - Validate the Forgot passward? Link and Reset Passward modal", () => {
...
it("Test Case 06 - Validate the Reset Password form submission", () => {
    cy.get(':nth-child(4) > a').click()
    cy.get('#email').type('jibrilmusleh@gmail.com')
    cy.get('#submit').click()
